# AGENTS.md — instructions for the AI engineer
...
2. **Start narrow — and note this rule is currently contradicted.** It was
   written as "Amazon sellers only, no multi-marketplace". The code now has
   three channels (Shopify, Amazon SP-API + Ads, WooCommerce), and only Shopify
   has ever run against a live account. Nothing has earned revenue yet.
   **This is an open product decision, not a settled one.** Do not add a fourth
   channel; either narrow back to the channel that can be verified, or rewrite
   this rule deliberately. Say which when it is decided.
