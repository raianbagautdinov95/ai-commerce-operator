...
export default function ProductFinder() {
  ...
  async function onEvaluate() {
    ...
    setError(null);
    try {
      const { results: r, weights: w } = await evaluateProducts(forms, true);
      setResults(r);
      setWeights(w);
      loadHistory();
    } catch (e) {
    ...
    <main className="px-6 pb-20 lg:px-11">
      <section className="pb-9 pt-11">
        <div className="flex items-center gap-3">
          <IconPlate name="gem" />
          <p className="lbl">Product hunter</p>
        </div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        ...
        <button onClick={addProduct} className="btn-quiet">+ ADD PRODUCT</button>
      </div>
