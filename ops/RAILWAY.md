...
Three services from one repository, plus Railway's PostgreSQL and Redis.

| Service    | Root directory | Config-as-code path      |
|------------|----------------|--------------------------|
| `api`      | `/backend`     | `railway/api.toml`       |
| `worker`   | `/backend`     | `railway/worker.toml`    |
| `frontend` | `/frontend`    | `railway/frontend.toml`  |

The config file path is set per service and is addressed from the repository
root — it does **not** follow the Root Directory. That is what lets `api` and
`worker` share `/backend` and still have different commands.
...
subdomains:
    ...
    app.eucompliancehq.com  → frontend
    api.eucompliancehq.com  → api

Shopify then gets:

    redirect  https://api.eucompliancehq.com/api/integrations/shopify/callback
    webhook   https://api.eucompliancehq.com/api/webhooks/shopify
...
a real store connects:
    ...
    CORS_ALLOWED_ORIGINS=https://app.eucompliancehq.com
    SHOPIFY_REDIRECT_URI=https://api.eucompliancehq.com/api/integrations/shopify/callback
    SHOPIFY_WEBHOOK_URI=https://api.eucompliancehq.com/api/webhooks/shopify
    LEGAL_ENTITY / LEGAL_ADDRESS / PRIVACY_CONTACT

And on the frontend service, as a **build** variable:

    NEXT_PUBLIC_API_BASE=https://api.eucompliancehq.com
